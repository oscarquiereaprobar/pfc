  ...
  onImageSelected(event: any): void {
    if (event?.target?.files && event.target.files.length > 0) {
      ...
      if (this.isEdit && this.previousImage && this.previousImage.startsWith('img_')) {
        this.http.delete(`https://desplieguedaw-hhhsfvaxa0ggardh.spaincentral-01.azurewebsites.net/api/itineraries/imagen/${this.previousImage}`).subscribe();
      }

      const formData = new FormData();
      formData.append('file', file, uniqueName);

      this.http.post('https://desplieguedaw-hhhsfvaxa0ggardh.spaincentral-01.azurewebsites.net/api/itineraries/upload', formData, { responseType: 'text' })
        .subscribe({
          next: (res: any) => {
